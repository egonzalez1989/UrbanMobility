import math, agentpy as ap, numpy as np
from UrbanUtils import weighted_astar, manhattan, euclidean

#######################################
# Intended as an interface fo agents in the city
#######################################
class UrbanAgent(ap.Agent):
    def setup(self):
        self.agent_type = "Urban"
        self.city = self.model.city
        self.next_action = None
        self.spawn_time = self.model.t

    # Position of the agent in the environment
    def get_position(self):
        return self.city.positions[self]

    ###########################################################
    # Basic agent's behavioral methods
    def sense(self):
        pass

    def react(self):
        pass

    def choose_action(self):
        pass

    def execute(self):
        pass
    ###########################################################



#######################################
# A GENERAL CLASS FOR URBAN AGENTS, PEDESTRIANS AND CARS UP TO NOW
#######################################
class MobileAgent(UrbanAgent):
    # Static maximum possible speed for any agent
    SPEED_LIMIT = 60
    INSTRUCTIONS = ['N', 'S', 'E', 'W', 'NE',  'NW', 'SE', 'SW', 'V']
    DIRECTIONS = [(-1, 0), (1, 0), (0, 1), (0, -1), (0, 0)]
    INSTRUCTION_MAP = dict(zip(INSTRUCTIONS, DIRECTIONS))
    DIRECTION_MAP = dict(zip(DIRECTIONS, INSTRUCTIONS))

    '''
    Initializing agent elements:
    - moves: action-movement dictionary
    - city: reference to its environment
    - goal: location of agent's destination
    - cost: total cost of travel. Up to now, riskier decisions produce higher cost
    - until_move: number of steps to wait for the movement to happen
    '''
    def setup(self, start, goal, speed, visibility, awareness):
        super().setup()
        self.start = start                          # (int, int): Initial position
        self.goal = goal                            # (int, int): Target cell
        self.speed = self.max_speed = speed         # Float: Maximum speed for this agent.
        self.active = True                          # bool: Agents are active by default. They are inactive when collisions or goal reached
        self.removal_time = math.inf                # int: Number of steps before removing from environmnet.
        self.conflicts = []
        self.real_position = start  # (float, float): A floating point positions datum instead of Grid's int coordinates
        self.next_positions = []
        self.direction = (0, 0)  # (int, int): One of 4 directions corresponding to: North, South, East, West
        self.visibility = visibility  # int: Number of neighbor cells of visivility (semi-accesible environment)
        self.awareness = awareness
        self.future_steps = 10

    #Starting
    def initialize_agent(self):
        # Initializae route and instructions
        self.find_route()

    ###########################################################
    # Sensing and reacting logic. Default behaviour:
    # - Return all agents found in the path and visibility of self
    def sense(self):
        # Get all agents in the visible neighborhood
        neighbors = self.city.neighbors(self, self.visibility)
        #   A conflict is found if trajectories are in risk of collision
        conflicts = list(filter(lambda x: x, [self.determine_conflict(x) for x in neighbors]))
        # Sort conflicts by distance, and pedestrian priority
        #self.conflicts = sorted(conflicts, key=lambda x: (manhattan(self.get_position(), x[1].get_position()), not x[1].agent_type == 'walker'))
        self.conflicts = sorted(conflicts, key=lambda x: manhattan(self.real_position, x[1].real_position))
        return self.conflicts

    # Determines if there is a possible collision with other agent. Default ignores any interaction
    def determine_conflict(self, other):
        return False

    def intent_move(self, real_position, direction):
        hjump, wjump = [direction[i] * self.speed / MobileAgent.SPEED_LIMIT for i in (0, 1)]
        next_real = (real_position[0] + hjump,
                     real_position[1] + wjump)
        next_pos = (int(next_real[0] + .5), int(next_real[1] + .5))
        return next_real, next_pos

    ################################################################################
    def choose_action(self):
        if self.active:
            self.next_action = self.instructions[self.get_position()]

    def execute(self):
        # If agent is active, execute action
        if self.active:
            self.move()

        # Otherwise check for removal
        else:
            if self.removal_time == 0:
                self.model.schedule_removal(self)
            elif self.removal_time > 0:
                self.removal_time -= 1
            else:
                raise ValueError("Removal time is negative! Verify implementation")

    # Actual movement according to speed
    def move(self):
        # Choose action according to current route plan
        self.direction = MobileAgent.INSTRUCTION_MAP[self.next_action]
        d = self.speed / MobileAgent.SPEED_LIMIT
        self.real_position, grid_position = self.intent_move(self.real_position, self.direction)
        self.city.move_to(self, grid_position)

        # If goal is reached, remove from environment
        if self.get_position() == self.goal:
            self.deactivate(removal_time=0)
            return

        # Update visibility
        self.set_next_positions()

    ################################################################################
    # Get next cost of moving to a specific cell
    def step_cost(self, position):
        cell_type = self.city.city_grid[position][0]
        return self.cost_map[cell_type]

    def step_risk(self, action, position):
        return 0

    # Deactivating the agent. Is not functional anymore and should be removed.
    def deactivate(self, removal_time = np.inf):
        self.active = False
        self.removal_time = removal_time

    # Default mobile agent reactions for accelerating and slowdown
    def react(self, conflicts = []):
        conflicts = self.conflicts if conflicts is None else conflicts
        if len(conflicts) == 0:
            cur_speed = self.speed
            self.speedup()
            acc = self.speed - cur_speed
            # Check if speedup does nos involve new collisions
            self.set_next_positions()
            new_conflicts = self.sense()
            if new_conflicts:
                if not new_conflicts[0][1].active:
                    self.reroute(lambda x: not x.active)
                else:
                    self.speed = self.speed - acc
                self.set_next_positions()
        else:
            self.slowdown()

    def slowdown(self):
        self.speed = 0

    def speedup(self):
        self.speed = self.max_speed
    #########################################################





class AstarAgent(MobileAgent):
    def setup(self, start, goal, speed, visibility, awareness, weight):
        super().setup(start, goal, speed, visibility, awareness)
        self.weight = weight

    # Get a list of instructions self.instructions to achieve the goal. A small list of next positions will update at each step to simulate visibility
    def find_route(self, grid = None):
        route = weighted_astar(self, self.get_position(), self.goal, MobileAgent.INSTRUCTION_MAP, grid=grid, heuristic=manhattan, weight=self.weight)
        dir_list = [self.DIRECTION_MAP[(route[i+1][0] - route[i][0], route[i+1][1] - route[i][1])] for i in range(len(route) - 1)]
        self.instructions = dict(zip(route[:-1], dir_list))
        self.instructions[self.goal] = 'V'
        self.set_next_positions()

    def set_next_positions(self):
        # Initializing next near positions (visibility)
        pos = self.real_position
        self.next_positions, self.next_real = [], []
        if self.get_position() == self.goal:
            return
        self.direction = dir = self.INSTRUCTION_MAP[self.instructions[self.get_position()]]
        for i in range(self.future_steps):
            new_pos, new_grid = self.intent_move(pos, dir)
            self.next_positions.append(new_grid)
            self.next_real.append(new_pos)
            if euclidean(new_pos, self.get_position()) > self.visibility or len(set(self.next_positions)) >= self.visibility:
                break
            if new_grid != self.goal:
                dir = self.INSTRUCTION_MAP[self.instructions[new_grid]]
                pos = new_pos

    # Compute a new route, considering obstacles
    def reroute(self, is_obstacle = lambda x: False):
        # Get blocking agents within the visibility field. Consider function is_obstacle to determine blocking agents
        # and update the city_grid
        neighbors = self.city.neighbors(self, self.visibility)
        blocking_agents = list(filter(is_obstacle, neighbors))
        #self_grid = self.city.city_grid[:, :]
        for blocking_agent in blocking_agents:
            self.obstacles.add(blocking_agent.get_position())
        self.find_route()
        for blocking_agent in blocking_agents:
            if blocking_agent.get_position() in self.obstacles:
                self.obstacles.remove(blocking_agent.get_position())

    # Check immediate route positions of agents to verify possible collisions
    def determine_conflict(self, other):
        n = min(len(self.next_positions), len(other.next_positions))
        if self.next_positions and (self.next_positions[0] == other.get_position()):
            return other.get_position(), other
        for i in range(1, n-1):
            for j in range(i-1, i+2):
                if euclidean(self.next_real[i], other.next_real[j]) < .7:
                    return other.next_positions[j], other

            #area, j = set([other.get_position()] if i == 0 else [other.next_positions[i-1]]), i
            #while j < len(other.next_positions) and len(area) < 3:
            #    area.add(other.next_positions[j])
            #    j += 1
            #if self.next_positions[i] in area:
        return False